    ...
    ****document = *text;
    ...
    while(*(text) != '\0') {
        ...
        if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
            ...
            *(*(*(document + num_paragraphs - 1) + num_sentences - 1) + num_words - 1) = (char *)realloc(*(*(*(document + num_paragraphs - 1) + num_sentences - 1) + num_words - 1), sizeof(char) * current_word_length);
            *(*(*(*(document + num_paragraphs - 1) + num_sentences - 1) + num_words - 1) + current_word_length - 1) = c;
        }
